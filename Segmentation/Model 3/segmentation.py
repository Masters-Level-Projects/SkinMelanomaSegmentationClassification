...
class SpatialDatasetNames(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.flags[index] == 1:
            ...
            temp_x = transformationPIL(temp_x)
            temp_y = transformationPIL(temp_y)
            temp_x = transforms.functional.adjust_contrast(temp_x, contrast_factor=1.2)
            temp_x = transforms.functional.adjust_gamma(temp_x, gamma=1.1, gain=1)
            temp_x = transformationTensor(temp_x)
            temp_y = transformationTensor(temp_y)
...
def preliminary_processing(path):
    ...
    #X_train = numpy.load(path+'/X_train.npy')
    #Y_train = numpy.load(path+'/Y_train.npy')
    #X_test = numpy.load(path+'/X_test.npy')
    #Y_test = numpy.load(path+'/Y_test.npy')
    #X_validation = numpy.load(path+'/X_val.npy')
    #Y_validation = numpy.load(path+'/Y_val.npy')
    #numpy.save(path+'/X_train.npy',X_train)
    #numpy.save(path+'/Y_train.npy',Y_train)
    #numpy.save(path+'/X_test.npy',X_test)
    #numpy.save(path+'/Y_test.npy',Y_test)
    #numpy.save(path+'/X_val.npy',X_validation)
    #numpy.save(path+'/Y_val.npy',Y_validation)
...
def define_model_nParameters(device,batch_size,test_batch_size,train_set,validation_set,learning_rate,adam_weight_decay,lr_patience):
    ...
    model = torch.load("/content/drive/My Drive/Segmentation2/learned_SkinLesion_Segmentation_Model.pt")
...
def learning(model,device,loss,optimizer,scheduler,n_epochs,train_loader,validation_loader,es_patience):
    ...
    for epoch in range(1, n_epochs + 1):
        ...
        if validation_loss <= old_validation_loss:
            validation_decrease_counter = 0
            torch.save(model,"/content/drive/My Drive/Segmentation2/checkpoint_Model.pt")
            old_validation_loss = validation_loss
        else:
            validation_decrease_counter += 1
        if es_patience <= validation_decrease_counter:
            model = torch.load("/content/drive/My Drive/Segmentation2/checkpoint_Model.pt")
        ...
        #numpy.save('/content/drive/My Drive/Segmentation2/t1.npy',numpy.asarray(train_loss_list))
        #numpy.save('/content/drive/My Drive/Segmentation2/t2.npy',numpy.asarray(validation_loss_list))
        #numpy.save('/content/drive/My Drive/Segmentation2/t3.npy',numpy.asarray(train_accuracy_list))
        #numpy.save('/content/drive/My Drive/Segmentation2/t4.npy',numpy.asarray(validation_accuracy_list))
...
def plot_losses(train_losses,validation_losses):
    ...
    plt.tight_layout()
    plt.show()
    plt.savefig('/content/drive/My Drive/Segmentation2/loss.jpg')
...
def plot_IoU(train_IoU,validation_IoU):
    ...
    plt.tight_layout()
    plt.show()
    plt.savefig('/content/drive/My Drive/Segmentation2/iou.jpg')
...
def saveImages(fileName,data,target,predicted):
    ...
    path = '/content/drive/My Drive/Segmentation2'
...
def model_segmentation():
    ...
    numpy.save('/content/drive/My Drive/Segmentation2/train_losses.npy',train_losses)
    validation_losses = numpy.asarray(validation_losses)
    numpy.save('/content/drive/My Drive/Segmentation2/validation_losses.npy',validation_losses)
    train_IoU = numpy.asarray(train_IoU)
    numpy.save('/content/drive/My Drive/Segmentation2/train_IOU.npy',train_IoU)
    validation_IoU = numpy.asarray(validation_IoU)
    numpy.save('/content/drive/My Drive/Segmentation2/validation_IOU.npy',validation_IoU)
    
    # Saving the learned Model
    print('Saving the model...')
    torch.save(learned_model,"/content/drive/My Drive/Segmentation2/learned_SkinLesion_Segmentation_Model.pt")
